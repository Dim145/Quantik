...
class ActionQuantik
{
    ...
    public function __toString():string
    {
        ...
        for ($cpt = 0; $cpt < PlateauQuantik::NBROWS; $cpt++ )
            $res .= "Ligne " . $cpt . ( ($this->isRowWin($cpt) ) ? " gagnante" : " perdante") . "<br/>";

        for ($cpt = 0; $cpt < PlateauQuantik::NBCOLS; $cpt++ )
            $res .= "Colonne " . $cpt . ( ($this->isColWin($cpt) ) ? " gagnante" : " perdante") . "<br/>";

        for ($cpt = 0; $cpt < 4; $cpt++ )
            $res .= "coté " . $cpt . ( ($this->isCornerWin($cpt) ) ? " gagnant" : " perdant") . "<br/>";
